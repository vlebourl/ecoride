...
type VehiclePageUser = NonNullable<NonNullable<ReturnType<typeof useProfile>["data"]>["user"]>;
type VehicleUpdateProfile = ReturnType<typeof useUpdateProfile>;
type VehicleBleController = ReturnType<typeof useSuper73>;

interface VehicleSettingsContentProps {
  user: VehiclePageUser;
  updateProfile: VehicleUpdateProfile;
  ble: VehicleBleController;
}

function VehicleSettingsContent({ user, updateProfile, ble }: VehicleSettingsContentProps) {
  const t = useT();
  const [autoModeEnabled, setAutoModeEnabled] = useState(user.super73AutoModeEnabled ?? false);
  ...
  const [autoModeHighSpeed, setAutoModeHighSpeed] = useState(
    String(user.super73AutoModeHighSpeedKmh ?? 17),
  );
      ...
      <section className="rounded-2xl bg-surface-container p-4">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-lg bg-surface-high p-3">
            ...
            <p className="mt-2 text-xs text-text-dim">{t("vehicle.defaults.thresholdsExplain")}</p>
          </div>
...
export function VehiclePage() {
  ...
  return (
    <>
      ...
      />
      <VehicleSettingsContent key={user.id} user={user} updateProfile={updateProfile} ble={ble} />
    </>
  );
}
